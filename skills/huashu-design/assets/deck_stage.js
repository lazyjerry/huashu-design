 ...
 * - 左右點擊區域導覽
 * - slide 計數器（目前/總數）
 * - localStorage 持久化目前 slide
 * - Speaker notes postMessage（支援外層渲染）
 * - Hash 導覽（#slide-5 跳到第 5 張）
 * - Print-to-PDF 支援（Cmd+P / Ctrl+P 一頁一 slide）
 * - 自動為每個 slide 加上 data-screen-label
 *
 * 用法：
 *   <deck-stage>
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // parser 此時可能尚未處理完子 <section>，querySelectorAll 會回傳空陣列。
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // 文件仍在 parse，等 DOMContentLoaded 一次處理完所有 section
